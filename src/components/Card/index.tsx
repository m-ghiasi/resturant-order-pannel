...
export default function Card(props: CardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl w-[22%] min-w-[180px] flex flex-col items-center justify-between gap-1 p-3 min-h-[300px] ">
      <div className="relative">
        <img src={image} alt={name} className="w-25 rounded-full" />
        {!isVegan ? (
          <img
            className="w-10 absolute top-0 right-[-50px]"
            src={vegan}
            alt=""
          />
        ) : null}
